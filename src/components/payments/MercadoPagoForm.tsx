...
const MercadoPagoForm: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const initMP = () => {
      console.log("initMP llamado, window.MercadoPago:", typeof window.MercadoPago);
      ...
      console.log("Inicializando MercadoPago con key:", import.meta.env.VITE_MP_PUBLIC_KEY?.slice(0, 15));
      ...
      const cardForm = mp.cardForm({
        ...
        form: {
          id: "mp-form",
          cardNumber:     { id: "mp-cardNumber",     placeholder: "1234 1234 1234 1234" },
          expirationDate: { id: "mp-expirationDate", placeholder: "MM/AA" },
          securityCode:   { id: "mp-securityCode",   placeholder: "CVV" },
          cardholderName: { id: "mp-cardholderName", placeholder: "Como aparece en la tarjeta" },
          issuer:         { id: "mp-issuer" },
        ...
        callbacks: {
          ...
          onSubmit: async (event: any) => {
            ...
            try {
              ...
              console.log("Token generado:", token, "PaymentMethod:", paymentMethodId);
  ...
  return (
    <form id="mp-form" className={styles.form}>
      ...
      <div style={{ visibility: mpReady ? "visible" : "hidden", height: mpReady ? "auto" : 0 }}>
      ...
      <button type="submit" className={styles.btn} disabled={loading || !mpReady}>
        {loading
          ? <span className={styles.spinner}><span className={styles.spinnerIcon} /> Procesando…</span>
          : "Confirmar y Pagar"
        }
      </button>
